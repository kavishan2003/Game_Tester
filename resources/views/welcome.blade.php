...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
   ...
   <body class="bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen text-gray-800 antialiased font-sans">
    ...
    <div class="container mx-auto p-4 sm:p-6 lg:p-8 max-w-6xl">
        ...
        <header class="flex items-center justify-center mb-12 mt-8">
            <i class="fas fa-gamepad text-5xl text-blue-600 mr-4"></i>
            <h1 class="text-5xl font-extrabold text-gray-900 drop-shadow-sm">🎮 Game Tester</h1>
        </header>

        <section class="mb-12">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-6 rounded-2xl shadow-xl flex flex-col items-center text-center transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
            <div x-data="{ activeTab: 'inProgress' }" class="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-8 rounded-2xl shadow-2xl flex flex-col justify-between">
                <div>
                    <h2 class="text-xl font-semibold mb-2 opacity-90">Your Current Wallet</h2>
                    <p class="text-6xl font-extrabold mb-6 drop-shadow-lg">$0.00</p>
                </div>
                <div class="flex space-x-3">
                    <button @click="activeTab = 'history'"
                            :class="activeTab === 'history' ? 'bg-white text-blue-700' : 'bg-white bg-opacity-20 text-white hover:bg-opacity-30'"
                            class="flex-1 py-3 px-6 rounded-full font-medium transition-all duration-200 shadow-md">
                        History
                    </button>
                    <button @click="activeTab = 'inProgress'"
                            :class="activeTab === 'inProgress' ? 'bg-white text-blue-700' : 'bg-white bg-opacity-20 text-white hover:bg-opacity-30'"
                            class="flex-1 py-3 px-6 rounded-full font-medium transition-all duration-200 shadow-md">
                        In Progress
                    </button>
    ...
    <div
        ...
        <img
            src="{{ $game['thumbnail'] }}"
            alt="{{ $game['title'] }}"
            class="w-32 h-32 object-cover mb-4 border-4 border-red-500 shadow-lg"
        >
    ...
    <div id="jackpotModal-{{ $index }}" class="fixed inset-0 bg-white/70  flex items-center justify-center p-4 hidden z-50">
        <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-auto overflow-hidden transform transition-all duration-300 scale-95 opacity-0" id="modalContent-{{ $index }}">
            <!-- Modal Header -->
            <div class="flex justify-between items-center bg-gradient-to-r from-green-700 to-green-600 text-white px-6 py-4 rounded-t-xl shadow-md">
                <h2 class="text-2xl font-bold">{{ $game['title'] }}</h2>
                <button class="closeModalBtn text-white hover:text-gray-200 focus:outline-none transition-transform duration-200 transform hover:scale-110" data-index="{{ $index }}">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <!-- Modal Body -->
            <div class="modal-body-scrollable p-4 max-h-[80vh]">
                <div class="mb-4 rounded-lg overflow-hidden shadow-lg">
                    <img src="{{ $game['thumbnail'] }}" alt="Game Preview" class="w-full h-auto object-cover rounded-lg">
                </div>

                {{-- <p class="text-gray-700 text-sm mb-6 text-center leading-relaxed">
                    Play now and hit a <span class="font-bold text-green-600">JACKPOT</span> with Jackpotland!
                    Complete each event to earn.
                </p> --}}
                  ...
                  @foreach($game['events'] as $task)
                    <div class="space-y-4 mb-6">
                        <div class="flex items-center justify-between bg-gray-50 border p-3 rounded-lg shadow-sm">
                            <span>{{ $task['name'] }}</span><span class="text-green-600 font-semibold">{{ $task['points'] }}</span>
                        </div>
                    </div>
                  @endforeach
                <div class="mb-4 mt-3">
                    <button class="btn-gradient w-full py-4 text-white font-bold text-lg rounded-xl shadow-lg transition-all duration-300 transform hover:scale-105">
...
@endforeach
    ...
    <!-- Button to trigger the modal (for demonstration) -->
    {{-- <button id="openModalBtn" class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-75 transition-all duration-300">
        Open Jackpotland Modal
    </button> --}}

    {{-- <!-- Modal Overlay -->
    <div id="jackpotModal" class="fixed inset-0 bg-white/70  flex items-center justify-center p-4 hidden z-50">
        <!-- Modal Content -->
        <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-auto overflow-hidden transform transition-all duration-300 scale-95 opacity-0" id="modalContent">
            <!-- Modal Header -->
            <div class="flex justify-between items-center bg-gradient-to-r from-green-700 to-green-600 text-white px-6 py-4 rounded-t-xl shadow-md">
                <h2 class="text-2xl font-bold">{{ $game['title'] }}</h2>
                <button id="closeModalBtn" class="text-white hover:text-gray-200 focus:outline-none transition-transform duration-200 transform hover:scale-110">
                    <!-- Custom Close Icon (X mark) -->
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <!-- Modal Body (with potential scroll for smaller screens/more content) -->
            <div class="modal-body-scrollable p-4 max-h-[80vh]">
                <!-- Image Section -->
                <div class="mb-4 rounded-lg overflow-hidden shadow-lg">
                    <img src="{{ $game['thumbnail'] }}" alt="Jackpotland Game Preview" class="w-full h-auto object-cover rounded-lg" onerror="this.onerror=null; this.src='https://placehold.co/600x400/28a745/ffffff?text=Image+Not+Found';">
                </div>
                ...
                <div class="space-y-4 mb-6">
                    ...
                    <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm">
                        <span class="text-gray-800 font-medium">Install</span>
                        <span class="text-green-600 font-semibold">$0.01</span>
                    </div>
                    <!-- Task Item 2 -->
                    <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm">
                        <span class="text-gray-800 font-medium">Reach Level 150</span>
                        <span class="text-green-600 font-semibold">$0.21</span>
                    </div>
                    <!-- Task Item 3 -->
                    <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm">
                        <span class="text-gray-800 font-medium">Reach Level 250</span>
                        <span class="text-green-600 font-semibold">$0.62</span>
                    </div>
                    <!-- Task Item 4 -->
                    <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm">
                        <span class="text-gray-800 font-medium">Purchase any $4.99+ Package</span>
                        <span class="text-green-600 font-semibold">$3.25</span>
                    </div>
                    <!-- Task Item 5 -->
                    <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm">
                        <span class="text-gray-800 font-medium">Collect Final Prize in Welcome Week Activity</span>
                        <span class="text-green-600 font-semibold">$1.82</span>
                    </div>
                    <div class="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-lg p-3 shadow-sm">
                        <span class="text-gray-800 font-medium">Purchase any $19.99+ Package</span>
                ...
                <!-- Main Action Button -->
                <div class="mb-4 mt-3">
                    <button class="btn-gradient w-full py-4 text-white font-bold text-lg rounded-xl shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-75 transition-all duration-300 transform hover:scale-105">
                        PLAY AND EARN $184.01
                    </button>
                </div>
